...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.maxpool(x1)  # 64 channels
...
class ResUNetBasedModel(nn.Module):
    def __init__(self):
        super(ResUNetBasedModel, self).__init__()
        self.encoder = ResNetEncoder()
        
        # Adjust RFB modules to match ResNet-34 feature dimensions
        self.rfb1 = RFB_modified(64, 64)   # Matches layer1
        self.rfb2 = RFB_modified(64, 64)   # Matches layer1
        self.rfb3 = RFB_modified(128, 64)  # Matches layer2
        self.rfb4 = RFB_modified(256, 64)  # Matches layer3
        self.rfb5 = RFB_modified(512, 64)  # Matches layer4
        
        # Decoder with adjusted input channels
        self.up1 = Up(128, 64)  # 64 (from rfb5) + 64 (from rfb4)
        self.up2 = Up(128, 64)  # 64 (from up1) + 64 (from rfb3)
        self.up3 = Up(128, 64)  # 64 (from up2) + 64 (from rfb2)
        self.up4 = Up(128, 64)  # 64 (from up3) + 64 (from rfb1)
    ...
    def forward(self, x):
        ...
        out1 = F.interpolate(self.side1(x), size=(352, 352), mode='bilinear', align_corners=True)
        
        x = self.up2(x, x3)
        # out2 = F.interpolate(self.side2(x), scale_factor=4, mode='bilinear')
        out2 = F.interpolate(self.side2(x), size=(352, 352), mode='bilinear', align_corners=True)
        
        x = self.up3(x, x2)
        # out = F.interpolate(self.head(x), scale_factor=2, mode='bilinear')
        out = F.interpolate(self.head(x), size=(352, 352), mode='bilinear', align_corners=True)
...
def main(args):
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch+1) % 5 == 0 or (epoch+1) == args.epoch:
            torch.save(model.state_dict(), os.path.join(args.save_path, f'ResUNet-{epoch+1}.pth'))
            print(f'[Saving Snapshot:] {os.path.join(args.save_path, f"ResUNet-{epoch+1}.pth")}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("ResUNet Training")
